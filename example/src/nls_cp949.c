...
#if defined(LANG_KOREAN_EXT_INCLUDE)
const wchar_t* const page_charset2uni[128] =
{
...
/**************************************************************************
*  FUNCTION NAME :
*      int char2uni_cp949(unsigned char *ansi, wchar_t *uni);
*
*  DESCRIPTION :
*  INPUT:
*			ansi	=
...
int char2uni_cp949(const unsigned char *ansi, wchar_t *uni)
{
	...
	{
		charset2uni = page_charset2uni[ch-0x80];

		if (ch >= 0xca)	// Chinese character of Korean
		{
			if (cl < 0xa0)
			{
				*uni = 0x0000;
		...
		else if (charset2uni != 0)	/* QAC : 13.2 */
		{
#if defined(LANG_KOREAN_EXT_INCLUDE)
